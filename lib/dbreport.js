  ...
  _setContext(aCtx) {
    ...
    _prepSchemaData(labelCounts) {
        ...
        this.schemaRecords.forEach( function (record) {
              ...
              // if relType is in Filter then both labels are also shown regardless the label filter
              // if relType is not in filter than the Label must be in the Filter to be shown

              if (current.relTypeFilter.includes(relType)) {
                  // both Labels and relationship
                  if (current._idIndex(nodes, startId) == null) {
                      if (startLabelCount > 0) {
                          calculatedSize = baseNodeSize + Number(Math.log10(startLabelCount).toFixed(2));
                      }
                      nodes.add({id: startId, label: startLabel + "\n " + startLabelCount,  title: startLabel, value : calculatedSize });
                      relatedLabels.push(startLabel);
                  }
                  if (current._idIndex(nodes, endId) == null) {
                      if (endLabelCount > 0) {
                          calculatedSize = baseNodeSize + Number(Math.log10(endLabelCount).toFixed(2));
                      }
                      nodes.add({id: endId, label: endLabel  + "\n " + endLabelCount,  title: endLabel, value : calculatedSize } );
                  ...
                  links.add({ from:startId, to:endId, label: relType, title:relType, arrows:'to', font: {align: 'top', size:10 }, color: {inherit : true}, length : 300 , smooth :  {enabled: true, type: 'curvedCW', roundness: 0.1} });
            }
            }
        );
        labelCounts.forEach( function (value, key, map) {
            if (!relatedLabels.includes(key) && current.labelFilter.includes(key)) {
                let calculatedSize = baseNodeSize;
                if ( value > 0) {
                    calculatedSize = baseNodeSize + Number(Math.log10(value).toFixed(2));
                }
                nodes.add({id: lonelyLabelNumber, label: key + "\n " + value ,  title: key, value : calculatedSize });
                lonelyLabelNumber++;
            }
        })  ;
