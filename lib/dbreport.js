...
class DBReport {
    ...
    async _initFilter (type) {
        ...
        if (doLabels) {
            this.labelFilter = await this._getLabels();
            // all labels
            // build label color map
            this._buildColorMap(this.labelFilter);

            let htmlLabelFilter = document.getElementById("labelFilter");
            htmlLabelFilter.innerHTML = this._createCheckTable(this.labelFilter, 'label');
        }
        if (doRelations) {
            // relation list
            this.relTypeFilter = await this._getRelationshipTypes();
            let htmlRelTypeFilter = document.getElementById("reltypeFilter");
            htmlRelTypeFilter.innerHTML = this._createCheckTable(this.relTypeFilter, 'rel');
        }
        if (doLabels || doRelations) {
            this.refreshModel();
        }
    }
    _createCheckTable(aList, listtype) {
        aList.sort();
        let c = "<div class='ui grid'>";
        for (let i = 0; i < aList.length; i++) {
            c += "<div class='three wide column'><div class='ui checkbox'><input checked=true name='cb" + listtype + "_filter_' value='" + aList[i] + "'  type=\"checkbox\" onChange=\"handleFilterChange(this)\"/><Label>" + aList[i] + "</Label></div></div>";
        }
        c += "</div>";
        return c;
    }
    ...
    liveCountAction(action) {
        ...
        if (action == "start") {
           ...
           this._disableButton("btStartLC");
           // enable pause button
           this._enableButton("btPauseLC");
           // enable stop button
           this._enableButton("btStopLC");
       }  else if (action == "pause") {
           this.liveCount = false;
           // enable start button
           this._enableButton("btStartLC");
           // disable pause button
           this._disableButton("btPauseLC");
       } else if (action == "stop") {
            this.liveCount = false;
    ...
    async _getLabels() {
        let rs = await this._runQuery(this.session, "call db.labels() yield label return label");
        let labels = [];
    ...
    async _getRelationshipTypes() {
        let rs = await this._runQuery(this.session, "call db.relationshipTypes() yield relationshipType return relationshipType");
    ...
    async analyseDB( aSampleTreshold, aSampleSize, aRandomFactor, analyzeLabelProps, analyzeRelProps, analyzeLabelCombo) {
        ...
        try {
            ...
            let relationShipTypes = await this._getRelationshipTypes();
            ...
            let lbls = await this._getLabels();
            ...
            let rpos;
            this.schemaRecords = this.resultRecords;
    ...
    // there maybe two processes at the same time firing queries, so the usage of a global result variable it not enough
    // the live count and the report can run at the same time. So a second 'results' is needed

    async _runQuery2(session, qry) {
        let current = this;
        if (current.analyzewithdebug) console.log(qry);

        const prom = session.run(qry);
        await prom.then( function(result) {
            current.resultRecords2 = result.records;
        })
            .catch(function (error) {
                let msg = error.message;
                if (msg.startsWith("There is no procedure with the name `db.schema`")) {
...
//        summaryhtml += "<div style='height : 200px'></div>";
        ...
        let crchart1 = c3.generate({
            bindto: '#chart1',
            data : { columns :[
                               ['count'].concat(this.chartseries)
                         ],
                    type: 'bar',
                    color: function(inColor, data) {
                        if(data.index !== undefined) {
                            return current._getLabelColor(current.chartlabels[data.index]);
                        }
    ...
    async _liveViewCount(current) {
        if (current.liveCount == true) {
            ...
            if (current.labelFilter.length > 0) {

                //  get all the labels
                let rs = await current._runQuery2(current.session, "call db.labels() yield label return label");
                let labelcntquery = "";
                let lii = 0;
                for (let i = 0; i < current.resultRecords2.length; i++) {
                    let lbl = current.resultRecords2[i].get("label").toString();
                    if (current.labelFilter.includes(lbl)) {
                ...
                ;
                let labelcount = current.resultRecords2.length;
                // label counts
                // let tStart = Date.now();
                rs = await current._runQuery2(current.session, labelcntquery);
                ...
                let labelitems = [];
                for (let lic = 0; lic < current.resultRecords2.length; lic++) {
                    let dataMap = current.resultRecords2[lic].get("data");
                    // check group
                    if (!current.labelcount_groups.get(dataMap.type)) {
                        current.labelcount_groups.add({id: dataMap.type, content: dataMap.type, style: "stroke: " + current._getLabelColor(dataMap.type) +";", options : { drawPoints : {styles : "stroke: " + current._getLabelColor(dataMap.type) +";" }}});
                    }
//                current.labelcount_dataSet.add(
                ...
                let options = current._timelineOptions(current.labelFilter.length, timeWindowStart, timeWindowEnd);
            ...
            if (current.relTypeFilter.length > 0) {
                // relationships
                let relcntquery = "";
                let rs = await current._runQuery2(current.session, "call db.relationshipTypes() yield relationshipType return relationshipType");
                let rii = 0;
                for (let ri = 0; ri < current.resultRecords2.length; ri++) {
                    let relType = current.resultRecords2[ri].get("relationshipType").toString();
                    if (current.relTypeFilter.includes(relType)) {
                        if (rii > 0) {
                ...
                // relationship count
                let relcount = current.resultRecords2.length;
                // label counts
                // tStart = Date.now();
                rs = await current._runQuery2(current.session, relcntquery);
                // loop over the query results and update the liveCountLabelData Map
                ...
                let options = current._timelineOptions(current.relTypeFilter.length, timeWindowStart, timeWindowEnd);
  ...
  _setContext(aCtx) {
	...
	_buildViz(lbllCounts) {
        // get the graph container
        if (this.network) {
            this.network.destroy(); // clear
        }
        ...
        this.graphContainer = document.getElementById("graphID");
        this.propsContainer = document.getElementById("propsID");

        this.gdata = this._prepSchemaData(simpleLabelCounts);
        // building vis
        this.voptions = { interaction : {selectConnectedEdges: false}, nodes : { color : { border : "black", background : "#66b14c", hover : { border : "blue"}, highlight: { border : "blue" }} , font : { size : 11},  shape : "dot" , scaling : { min : 5, max : 30}}};
        // default hierarchical
        this.voptions.layout = { hierarchical: {
                enabled:true,
                levelSeparation: 150,
                nodeSpacing: 100,
                treeSpacing: 200,
        ...
        this.network = new vis.Network(this.graphContainer, this.gdata, this.voptions);

        this._registerVizEvents();
    }
    _registerVizEvents() {
        this._clearProps();
        let current = this;
        this.network.on("selectNode", function (params)  {
            current._showNodeProps(params.nodes[0]);
        });
        ...
        this.network.on("selectEdge", function (params)  {
            ...
            current._showRelProps(params.edges[0]);
        });
    ...
    _showNodeProps(params) {
        let nodeName = this.gdata.nodes.get(params).title;
        ...
        html += "</div>";
        this.propsContainer.innerHTML = html;
    }
    ...
    _showRelProps(params) {
        let relType = this.gdata.edges.get(params).title;
    ...
    _redrawViz() {
        ...
        if (this.dbinfo) {
            if (!this.network) {
                // there is no network create is
                this._buildViz(this.dbinfo.labelCounts);
                this._clearProps();
                this._registerVizEvents();
            }
    ...
    _prepSchemaData(labelCounts) {
        let cbFR = document.getElementById("cb_FR").checked; // when checked relationships are always shown including connecting labels

        let nodeColor = "yellow";
        let nodes = new vis.DataSet();
        let links = new vis.DataSet();
        let current = this;
        let lonelyLabelNumber = 0;
        let relatedLabels = [];
        let baseNodeSize = Number(10);
        let calculatedSize = baseNodeSize;
        this.schemaRecords.forEach( function (record) {
              // snId is the startNode id
              // enId is the endNode id
              // startNode is a map with startNode info
              // endNode is amap with endNode info
              let startId = Math.abs(record.get("snId").toNumber()) + 1000; // leave room for labels without relations "lonelyLabels"

              let endId = Math.abs(record.get("enId").toNumber()) + 1000;
              let startLabel = record.get("startNode").properties.name;
              let startLabelCount = Number(labelCounts.get(startLabel));
              let endLabel = record.get("endNode").properties.name;
              let endLabelCount = Number(labelCounts.get(endLabel));
              let relType = record.get("relType").toString();
              // console.log({ startId: startId, startLabel : startLabel, relType : relType , endLabel: endLabel, endId : endId  });

              if (current.relTypeFilter.includes(relType)) {
                  // both Labels and relationship
                  if (cbFR) {
                      // force relationships. if reltype is in filter the labels are always shown
                      current._addRelationShip(startLabel, startId,startLabelCount, endLabel, endId,endLabelCount, relType, nodes, links, relatedLabels);
                  } else {
                      // if the relationship and start and end node are in filter then the structure is displayed
                      if (current.labelFilter.includes(startLabel) && current.labelFilter.includes(endLabel)) {
                          current._addRelationShip(startLabel, startId,startLabelCount, endLabel, endId,endLabelCount, relType, nodes, links, relatedLabels);
                      }
                  }
              }
        ...
        labelCounts.forEach( function (value, key, map) {
            if (!relatedLabels.includes(key) && current.labelFilter.includes(key)) {
                current._addNode(nodes, lonelyLabelNumber,key,value);
                lonelyLabelNumber++;
            }
        })  ;
        // check the labelCounts for if there are Labels without relationsihps
        // console.log(nodes);

        return { nodes: nodes, edges: links};
    }

  _getLabelColor(aLabel) {
        if (aLabel) {
            return this.colorMap.get(aLabel);
        } else {
            return "#66b14c";
        }
  }
  ...
  _addNode(nodes, aNodeId, aLabel, labelCount) {
      ...
      nodes.add({
          ...
          color: { border : "black", background : this._getLabelColor(aLabel), hover : { border : "blue"}, highlight: { border : "blue" }}
      });
  }
  ...
  _addRelationShip(startLabel, startId,startLabelCount, endLabel, endId,endLabelCount, relType, nodes, links, relatedLabels) {
    ...
    refreshModel() {
        //
        this._handleLayoutButtons("UD"); // default
        if (this.dbinfo) {
            this._buildViz(this.dbinfo.labelCounts);
        }
    ...
    changeLayout(layoutType) {
        ...
        if (layoutType) {
            if ( ["UD","DU","LR","RL"].includes(layoutType) ) {  // hierarchical
                if (this.voptions.layout) {
                    delete this.voptions.layout;
                    delete this.voptions.physics;
                }
                this.voptions.layout = { hierarchical: {
                        enabled:true,
                        levelSeparation: 150,
                ...
                this.voptions.layout.hierarchical.sortMethod = sortMethod;
            }
            else {
                if (this.voptions.layout) { // graph
                    delete this.voptions.layout;
                    delete this.voptions.physics;
                }
            }
        ...
        if (this.network) {
            this.network.setOptions(this.voptions);
            // reload the data
            //
            if ( ["UD","DU","LR","RL"].includes(layoutType) ) {
                this.network.setData(this.gdata);
            }
            this.network.redraw();
    ...
    _handleLayoutButtons(lt) {
        ...
        // if (lt == "graph") {
        //     this._disableButton("btGraphVis");
        //     this._disableButton("btH_DirViz");
        //     this._disableButton("btH_HubSizeViz");
        //     this._enableButton("btH_UDViz");
        //     this._enableButton("btH_DUViz");
        //     this._enableButton("btH_LRViz");
        //     this._enableButton("btH_RLViz");
        // }

        // UD
        if (lt == "UD") {
//            this._enableButton("btGraphVis");
            this._disableButton("btH_UDViz");
            this._enableButton("btH_DUViz");
            this._enableButton("btH_LRViz");
            this._enableButton("btH_RLViz");
            this._disableButton("btH_DirViz");
            this._enableButton("btH_HubSizeViz");
        }
