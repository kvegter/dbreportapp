...
class LiveCount {
    constructor(dbname, lcm) {
        ...
        this.session;
        this.lcm = lcm;
        this.msgElm = document.getElementById("msgElM_" + this.dbname );
    }

    async init() {
        if (this.nea) {
            await this._initFilterLive();
        } else {
            console.log("ERROR Livecount needs a neo4j Accessor Object");
        }
    }

    async _initFilterLive (type, doall) {
        let doAll = false;
        if (doall && doall == true) {
            doAll = true;
        }

        this.session = this.nea.getReadSession(this.dbname);
        // label list
        let doLabels = true;
        ...
        // console.log("_initFilterLive ")
        if (doLabels) {
            this.labelFilterLive = await this.nea.getLabels(this.session);
            this.labelCount = this.labelFilterLive.length;
            // all labels

            let htmlLabelFilter = document.getElementById("labelFilterLive_" + this.dbname);
            htmlLabelFilter.innerHTML = this._createLiveCheckTable(this.labelFilterLive, 'label', doAll);
        }
        if (doRelations) {
            // relation list
            this.relTypeFilterLive = await this.nea.getRelationshipTypes(this.session);
            this.relTypeCount = this.relTypeFilterLive.length;
            let htmlRelTypeFilter = document.getElementById("reltypeFilterLive_" + this.dbname);
            htmlRelTypeFilter.innerHTML = this._createLiveCheckTable(this.relTypeFilterLive, 'rel', doAll);
        }
        this._checkParameters();
    }

    _checkParameters() {
        let lblfilter = document.getElementById("lcl_" + this.dbname);
        let selectedLabels = 0;
        if (this.labelFilterLive) {
            selectedLabels = this.labelFilterLive.length;
        }
        lblfilter.innerHTML = "Labels (" + selectedLabels + "/" + this.labelCount + ")";

        let relfilter = document.getElementById("lcr_" + this.dbname);
        let selectedRels = 0;
        if (this.relTypeFilterLive) {
            selectedRels =  this.relTypeFilterLive.length;
        }
        relfilter.innerHTML = "Relationship Types (" + selectedRels + "/" + this.relTypeCount + ")";
    }


    _createLiveCheckTable(aList, listtype, doAll) {
        aList.sort();
        ...
        for (let i = 0; i < aList.length; i++) {
            if (i == 0 || doAll == true) {
                c += "<div class='three wide column'><div class='ui checkbox'><input checked=checked name='cb" + listtype + "_live_" + this.dbname + "' value='" + aList[i] + "'  type=\"checkbox\" onChange=\"dbreportApp.handleLiveChange('" + this.dbname + "' , this)\"/><Label>" + aList[i] + "</Label></div></div>";
            } else {
                c += "<div class='three wide column'><div class='ui checkbox'><input name='cb" + listtype + "_live_" + this.dbname + "' value='" + aList[i] + "'  type=\"checkbox\" onChange=\"dbreportApp.handleLiveChange('" + this.dbname + "' , this)\"/><Label>" + aList[i] + "</Label></div></div>";
    ...
    doScaleChange() {
        // useLogScale
        this.liveCountLogScale = document.getElementById("useLogScale_" + this.dbname).checked;
    ...
    async _liveViewCount(current) {
        if (current.liveCount == true) {
            let timeWindow = Number(document.getElementById("timeWindow_" + current.dbname).value); // in hours
            let timeout = Number(document.getElementById("liveViewRefreshRate_" + current.dbname).value) * 1000; // functions needs millis
            ...
            if (current.labelFilterLive.length > 0) {
                ...
                // label counts
                // let tStart = Date.now();
                rs = await current.nea.runQueryRecords(current.session, labelcntquery, null, this.msgElm);
                ...
                for (let lic = 0; lic < rs.length; lic++) {
                    ...
                    if (!current.labelcount_groups.get(dataMap.type)) {
                        current.labelcount_groups.add({id: dataMap.type, content: dataMap.type, style: "stroke: " + current.lcm.getLabelColor(dataMap.type) +";", options : { drawPoints : {styles : "stroke: " + current.lcm.getLabelColor(dataMap.type) +";" }}});
                    }
//                current.labelcount_dataSet.add(
                ...
                let clabelContainer = document.getElementById("clabellivecount_" + current.dbname);
            ...
            if (current.relTypeFilterLive.length > 0) {
                // relationships
                let relcntquery = "";
                let rs = await current.nea.runQueryRecords(current.session, "call db.relationshipTypes() yield relationshipType return relationshipType", null, this.msgElm);
                let rii = 0;
                ...
                // label counts
                // tStart = Date.now();
                rs = await current.nea.runQueryRecords(current.session, relcntquery, null, this.msgElm);
                // loop over the query results and update the liveCountLabelData Map
                ...
                // get the container for the vis.js timeline and determine height
                let crelContainer = document.getElementById("crellivecount_" + current.dbname);
    ...
    _liveCountClick(type, properties, dataSet) {
        let refreshRate = Number(document.getElementById("liveViewRefreshRate_" + this.dbname).value); // read it from input field
    ...
    liveCountAction(action) {
        // console.log("livecount action " + action + " in db " + this.dbname);
        // btStartLC
        // btPauseLC
        // btStopLC
        if (action == "start") {
            ...
            this._disableElement("btStartLC_" + this.dbname);
            // enable pause button
            this._enableElement("btPauseLC_" + this.dbname);
            // enable stop button
            this._enableElement("btStopLC_" + this.dbname);
        }  else if (action == "pause") {
            this.liveCount = false;
            // enable start button
            this._enableElement("btStartLC_" + this.dbname);
            // disable pause button
            this._disableElement("btPauseLC_" + this.dbname);
        } else if (action == "stop") {
            this.liveCount = false;
            ...
            // disable stop button
            this._disableElement("btStopLC_" + this.dbname);
            // enable start button
            this._enableElement("btStartLC_" + this.dbname);
            // disable pause button
            this._disableElement("btPauseLC_" + this.dbname);
    ...
    handleLiveChange(elm) {
        if (elm.name.startsWith("cblabel_live_" + this.dbname) ) {
            // label change
    ...
    _deselectAllFilterLive(type) {
        let elements = document.getElementsByName("cb" + type + "_live_" + this.dbname);
