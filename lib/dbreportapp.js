...
class DBReportApp {
        ...
        if (nac.neodb.majorVersion > 3) {
            for (let i = 0; i < nac.neodb.databases.length; i++) {
                ...
                if (dbinfo.currentStatus == "online" && dbinfo.name != "system") {
                    if (dbnamehash.indexOf(dbinfo.name) == -1) {
                        dbnames.push(dbinfo.name);
                        dbnamehash += "#" + dbinfo.name;
                    }
                }
            }
    ...
    exportSummary(dbname) {
        let repCNT = this.databaseApps.get(dbname);
        repCNT.dbco._exportSummary();
    }
    exportModel(dbname) {
        let repCNT = this.databaseApps.get(dbname);
        repCNT.dbco._exportModel();
    }
    ...
    showWalkerHelp(dbname) {
        let repCNT = this.databaseApps.get(dbname);
        repCNT.dbmo.showWalkerHelp();
    }
    // count properties
...
class ReportContainer {
    ...
    _buildHTML() {
        ...
        html += '<div class="ui top attached tabular menu">\n' +
            ...
            '                                </td>\n' +
            '                                <td>\n' +
            '                                    <button id="exportSummary_' + this.dbname + '" class="ui positive disabled compact basic icon button"  onclick="dbreportApp.exportSummary(\'' + this.dbname + '\');"><i aria-hidden="false" class="download icon"></i>&nbsp;Export Summary</button>\n' +
            '                                </td>\n' +
            '                                <td>\n' +
            '                                    <button id="exportModel_' + this.dbname + '" class="ui positive disabled compact basic icon button"  onclick="dbreportApp.exportModel(\'' + this.dbname + '\');"><i aria-hidden="false" class="download icon"></i>&nbsp;Export Model</button>\n' +
            '                                </td>\n' +
            '                            </tr>\n' +
            '                        </table>\n' +
            '                    </td>\n' +
