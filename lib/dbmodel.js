...
class DBModel {
    ...
    _buildWalkerViz(lbllCounts) {
        ...
        if (this.walkernetwork) {
            ...
            console.log(' _buildWalkerViz walker network destroyed');
        }
    ...
    _redrawWalkerViz() {
        ...
        if (dbreportApp.hasDBInfo(this.dbname)) {
            ...
            if (!this.walkernetwork) {
               // console.log('initializing walkernetwork from _redrawWalkerViz');
                this._clearWalkerProps();
    ...
    _checkNetwork() {
        if (dbreportApp.hasDBInfo(this.dbname)) {
            if (this.walkernetwork == undefined) {
                let dbinfo = dbreportApp.getDBInfo(this.dbname);
                this._clearWalkerProps();
                this._buildWalkerViz(dbinfo.labelCounts);
                this._registerWalkerVizEvents();
    ...
    _handleWalkerContext(nid, xpos, ypos) {
        ...
        let html = '<div class="ui secondary vertical menu" style="max-height: 400px; width: 250px; overflow:auto; background-color: #ffffff ;font-size: .7em; border: outset; position: absolute; z-index: 1000; left: ' + x + 'px; top: ' + y + 'px;">';
        if (nodeData.outgoingRelations && nodeData.outgoingRelations.length > 0) {
            ...
            for (let i = 0; i < nodeData.outgoingRelations.length; i++) {
                ...
                if (this._notInVis(nid, item.type, false) == true) {
                    ...
                    html += '<a class="item" style="background-color: #c9ebf6;" onClick="dbreportApp._removeWalkerRelationship(\'' + this.dbname + '\',' + nid + ',\'' + item.type + '\',false); "><i class="icon eraser"></i><i class="icon long arrow alternate right"></i>';
                    html += nodeData.outgoingRelations[i].type;
                    html += '</a>';
                }
            }
        ...
        if (nodeData.incomingRelations && nodeData.incomingRelations.length > 0) {
            for (let i = 0; i < nodeData.incomingRelations.length; i++) {
                ...
                if (this._notInVis(nid, item.type, true) == true) {
                    ...
                    html += '<a class="item" style="background-color: #c9ebf6;" onClick="dbreportApp._removeWalkerRelationship(\'' + this.dbname + '\',' + nid + ',\'' + item.type + '\',true); "><i class="icon eraser"></i><i class="icon long arrow alternate left"></i>';
                    html += nodeData.incomingRelations[i].type;
                    html += '</a>';
                }
            }
        }
        html+= '<a class="item" style="background-color: #c9ebf6;" onClick="dbreportApp._deleteWalkerNode(\'' + this.dbname + '\',\'' + nodeName + '\' ); "><i class="icon eraser"></i>Clear Label</a></div>';
    ...
    _addWalkerRelationship(nid, type, incoming) {
        // console.log("_addWalkerRelationship nid:" + nid + " type:" + " incoming" + incoming);
        this.walkerContext.innerHTML = "";
        let current = this;
        // walk now through the schema rels
        let dbinfo = dbreportApp.getDBInfo(this.dbname);
...
//        console.log(relType + " sameCount:" + sameCount  + ":roundness " + roundness);
    ...
    ___showWalkerDataInfo() {
        if (this.walkerdata) {
            console.log(" showCompleteSchema #nodes:" + this.walkerdata.nodes.length + " #edges:"  + this.walkerdata.nodes.length );
        } else {
            console.log(" showCompleteSchema no walkerdata yet ");
        }
    }
    ...
    async showCompleteSchema() {
        ...
        let current = this;
        let dbinfo = dbreportApp.getDBInfo(this.dbname);
        // this.___showWalkerDataInfo();
        if (dbinfo.complexityScore < 400) {
            this._clearWalkerProps();
            await this._selectAllLabels();
            await this._selectAllRelTypes();
            ...
            $(key)
                .modal({
                    ...
                    onApprove : async function() {
                        current._clearWalkerProps();
                        await current._selectAllLabels();
                        await current._selectAllRelTypes();
    ...
    _addRemoveWalkerRel(aType, aChecked) {
        // console.log(" _addRemoveWalkerRel aType:" + aType + " aChecked:" + aChecked );
        //
