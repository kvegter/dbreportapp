...
//        let neoInstanceName = this.desktopInstanceName;
        ...
        try {
            ...
            rs = await nac.runQueryRecords(this.session, "call dbms.functions() yield name as name " +
                "with name where name = 'apoc.meta.type' " +
            ...
            if (this.analyseLabelProps && this.analyseLabelProps == true) {
                ...
                for (b = 0; b < labelCounts.length; b++) {
                    ...
                    if (this.analysePropsLabels.includes(labelCounts[b].label)) {
                        ...
                        if (useSample) {
                            qp = "MATCH (n:`" + labelCounts[b].label + "`) where rand() < " + this.randomFactor + " with n limit " + this.sampleSize + " return keys(n), count(n) as cnt";
                        }
            ...
            if (this.analyseLabelCombo && this.analyseLabelCombo == true) {
                ...
                if (useSample) {
                    labelCombinationQuery = "match (n) where rand() < " + this.randomFactor + " and size(labels(n)) > 1  with n limit " + this.sampleSize + " return distinct labels(n) as labellist";
                }
            ...
            for (rcn = 0; rcn < relTypes.length; rcn++) {
                ...
                if (this.analyseRelProps) {
                    if (this.analysePropsRelTypes.includes(relTypes[rcn].type)) {
                        ...
                        for (let li = 0; li < labels.length; li++) {
                            if (li == 0) {
                                if (useRelationSample) {
                                    rpvraag = "match (n)-[r:`" + relTypes[rcn].type + "`]->() where rand() < " + this.randomFactor + " and ( n:`" + labels[li] + "` ";
                                } else {
                        ...
                        if (useRelationSample) {
                            rpvraag += ") with r limit " + this.sampleSize + " return keys(r), count(r) ";
                        } else {
...
//        let neoInstanceName = this.desktopInstanceName;
        ...
        try {
            ...
            let record = rs[0]; // we know it is always one
            // check if apoc is available
            let funcQuery = "call dbms.functions() yield name as name " +
                "with name where name = 'apoc.meta.type' " +
                "return count(name) as cnt";
            if (nac.neodb.majorVersion > 4 ) {
                funcQuery = "show functions yield name " +
                    " where name = 'apoc.meta.type' " +
                    "return count(name) as cnt";
            }
            rs = await nac.runQueryRecords(this.session, funcQuery,null, this.repContainer );
            ...
            //
            this._appendMessage( "analyzing database (4/" + steps + "): Indexes...", true);
            let qindexes = "call db.indexes()"
            if (nac.neodb.majorVersion > 4) {
                qindexes = "show indexes yield *"
            }
            rs = await nac.runQueryRecords(this.session, qindexes,null, this.repContainer);
            rs.forEach(function (rcd) {
                let lind = rcd.toObject();
                indexes.push(lind);
            });
            ...
            //
            this._appendMessage( "analyzing database (5/" + steps + "): Constraints...", true);
            let qconstraints = "call db.constraints()";
            if (nac.neodb.majorVersion > 4) {
                qconstraints = "show constraints yield *"
            }
            rs = await nac.runQueryRecords(this.session, qconstraints,null, this.repContainer );
            rs.forEach(function (rcd) {
                let lind = rcd.toObject();
                constraints.push(lind);
            });
...
// problems here!
                ...
                for (b = 0; b < labelCounts.length; b++) {
                    ...
                    if (this.analysePropsLabels.includes(labelCounts[b].label)) {
                        ...
                        for (allp = 0; allp < allProps.length; allp++) {
                            ...
                            if (apocAvailable) {
                                rs = await nac.runQueryRecords(this.session, "MATCH (n:`" + labelCounts[b].label + "`) where n.`" + prp + "`  IS NOT NULL return apoc.meta.type(n.`" + prp + "`) limit 1",null, this.repContainer);
                                propTypes.push(prp + " - " + rs[0].get(0) + " - " + this._schemaColumn(labelCounts[b].label, prp, indexes, constraints));
                            } else {
                                rs = await nac.runQueryRecords(this.session, "MATCH (n:`" + labelCounts[b].label + "`) where n.`" + prp + "` IS NOT NULL return n.`" + prp + "` limit 1",null, this.repContainer );
                                propTypes.push(prp + " - " + this._getValType(rs[0].get(0)) + " - " + this._schemaColumn(labelCounts[b].label, prp, indexes, constraints));
                            }
                        }
                        labelCounts[b].allProperties = propTypes;
            ...
            for (rcn = 0; rcn < relTypes.length; rcn++) {
                ...
                if (this.analyseRelProps) {
                    if (this.analysePropsRelTypes.includes(relTypes[rcn].type) && labelOutRelMap.has(relTypes[rcn].type)) {
                        ...
                        for (allp = 0; allp < allRelProps.length; allp++) {
                            ...
                            rptq += " ) and r.`" + prp + "` IS NOT NULL return r.`" + prp + "` limit 1";
                            rs = await nac.runQueryRecords(this.session, rptq, null, this.repContainer);
                            let val = rs[0].get(0);
    ...
    _schemaColumnLegacy(aLabelorRelType, aProperty, indexes, constraints) {
        let res = "";
        let multiple = false;
    ...
    _schemaColumnLegacy5Plus(aLabelorRelType, aProperty, indexes, constraints) {
        let res = "";
        let multiple = false;

        // use the constraints first
        // we use the contraints to find the mandatory contraints

        if (constraints && constraints.length > 0) {
            for (let i = 0; i < constraints.length; i++) {
                let constraint = constraints[i];
                // is the current label type or property applicable?
                if (constraint.labelsOrTypes.indexOf(aLabelorRelType) > -1
                    && constraint.properties.indexOf(aProperty) > -1) {
                    // the current label/type property combination is in this constraint
                    if (constraint.type.indexOf("PROPERTY_EXISTENCE") > 0 ) {
                        res = res + 'M';
                    ...
                    } else if (constraint.type == "UNIQUENESS") {
                        if (constraint.properties.length > 1 ) {
                            res = "U*";
                        } else {
                            res = "U";
        ...
        // the indexes only the ones which are not based ona unique constraint
        if (indexes && indexes.length > 0) {
            // BTREE (Index and Uniqueness first
            for (let i = 0; i < indexes.length; i++) {
                let ind = indexes[i];
                if (ind.labelsOrTypes !== null && ind.labelsOrTypes.indexOf(aLabelorRelType) > -1
                    && ind.properties.indexOf(aProperty) > -1) {
                    if (ind.owningConstraint) {
                        if (ind.owningConstraint == null) {
                            if (ind.type == "FULLTEXT") {
                                res += "F";
                            } else {
    ...
    _schemaColumn(aLabelorRelType, aProperty, indexes, constraints) {
        ...
        if (nac.neodb.majorVersion < 5) {
            return this._schemaColumnLegacy(aLabelorRelType, aProperty, indexes, constraints)
        } else {
            return this._schemaColumnLegacy5Plus(aLabelorRelType, aProperty, indexes, constraints)
        }
    }
